...
public class SimpleFileRestClientV04
  ...
  public void uploadFile(String filePath) throws IOException {
    ...
    int chunkSize = (int) Math.min(bufferSize, Integer.MAX_VALUE); // maximal 2 GB
    ...
    // Original Dateiname wird im Header mitgesendet
    ...
    // Erfolgreiche Antwort lesen → UUID-Filename vom Server anzeigen
    try (InputStream in = conn.getInputStream()) {
      String result = new String(in.readAllBytes());
      System.out.println("Server response: " + result);

      if (result.startsWith("Uploaded as: ")) {
        String serverFileName = result.substring("Uploaded as: ".length()).trim();
        System.out.println("File was stored on server as: " + serverFileName);
      }
    }
  }
